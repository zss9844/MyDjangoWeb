...
import requests
from flask import Flask, make_response, request, json,render_template
from cx_homework2 import mobile_func

from cx_homework2.func import login
...
def workinfo():
    main_session = requests.session()
    uuid = request.cookies.get('uuid')  # TODO:production记得换回来
    # uuid = '00062f7b-ae33-41bc-bc34-c127b8a7f864'
    file_path = str('data/' + str(uuid) + '.dat')
    with open(file_path, 'r+') as f:
        user_dat = json.loads(f.read())
    main_session.cookies.update(user_dat['session'])
